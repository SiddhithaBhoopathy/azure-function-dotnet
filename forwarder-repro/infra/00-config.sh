#!/usr/bin/env bash
# Shared configuration for the FORWARDER-ONLY correlation reproduction.
...
export RESOURCE_GROUP="${RESOURCE_GROUP:-dd-fwd-repro-rg}"
export APP_SERVICE_PLAN="${APP_SERVICE_PLAN:-dd-fwd-repro-plan}"
...
export STORAGE_ACCOUNT="${STORAGE_ACCOUNT:-ddfwdrepro$RANDOM}"
# Function app name must be globally unique (becomes <name>.azurewebsites.net).
export FUNCTION_APP="${FUNCTION_APP:-dd-fwd-repro-$RANDOM}"
...
# COST TIP: a Dedicated P1V3 plan bills hourly. To avoid a second plan, reuse the existing
# repro's plan/RG by exporting them before running:
...
export DD_SERVICE="${DD_SERVICE:-azure-functions-fwd-repro}"
...
export REPRO_TAG_KEY="${REPRO_TAG_KEY:-dd-fwd-repro}"
